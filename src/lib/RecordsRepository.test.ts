import RecordsRepository, { type GameRecord } from 'lib/RecordsRepository';
...
const saveRecord = (rounds: number, player = 'Player'): void => {
    const record = RecordsRepository.createRecord(rounds);
    if (record) RecordsRepository.save({ record, player });
};
...
describe('RecordsRepository', () => {
    ...
    describe('save', () => {
        it('persists a record in localStorage', () => {
            ...
            if (record) RecordsRepository.save({ record, player: 'Bob' });
        ...
        it('returns true on successful save', () => {
            const record = RecordsRepository.createRecord(5) as GameRecord;
        ...
        it('removes the worst record when the leaderboard is full', () => {
            ...
            if (record) RecordsRepository.save({ record, player: 'Champion' });
