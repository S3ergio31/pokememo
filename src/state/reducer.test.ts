...
import type { GameState, GameAction } from 'state/reducer';
import Pokemon from 'lib/Pokemon';
import type { CardEntry } from 'lib/PokemonsGenerator';

const makeCards = (...ids: number[]): CardEntry[] =>
    ids.flatMap((id, i) => [
        { key: i * 2, value: new Pokemon(id) },
        { key: i * 2 + 1, value: new Pokemon(id) },
    ]);

const dispatch = (state: GameState, type: string, payload?: Pokemon): GameState =>
    gameReducer(state, { type, payload } as GameAction);
